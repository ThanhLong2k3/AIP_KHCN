...
const createAccountService = async (model) => {
    try {
        ...
        const existingUsername = await (0, account_repository_1.checkUsernameExists)(model.username.trim());
        if (existingUsername) {
        ...
        const decryptedPassword = (0, access_1.decrypt)(model.password);
...
const updateAccountService = async (model) => {
    try {
        ...
        if (isInvalidEmail)
            throw new Error('Địa chỉ email không được hỗ trợ');
        const currentUser = await (0, account_repository_1.authenticate)(model.username);
...
const registerOTPService = async (email, username) => {
    try {
        //kiểm tra username đã tồn tại chưa
        const existingUsername = await (0, account_repository_1.checkUsernameExists)(username.trim());
        if (existingUsername) {
...
const updateProfileService = async (username, model) => {
    try {
        ...
        await (0, account_repository_1.updateProfile)(updatedModel);
        // Sau khi update, gọi lại hàm authenticate để lấy tất cả thông tin mới
        const updatedAccount = await (0, account_repository_1.authenticate)(username);
        if (!updatedAccount || !updatedAccount[0]) {
            throw new Error("Không thể lấy thông tin tài khoản sau khi cập nhật.");
        }
        //lấy thông tin quyền
        const allPermissionsInfo = await (0, permission_repository_1.getPermissionsByRole)(updatedAccount[0].role_id);
        const permissions = allPermissionsInfo.map((p) => p.permission_code);
        return {
            success: true,
            user: {
